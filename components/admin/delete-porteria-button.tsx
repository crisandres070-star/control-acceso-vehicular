...
type DeletePorteriaButtonProps = {
    action: (formData: FormData) => void | Promise<void>;
    porteriaId: number;
    nombre: string;
};

export function DeletePorteriaButton({ action, porteriaId, nombre }: DeletePorteriaButtonProps) {
    ...
    return (
        <>
            <form action={action} ref={formRef}>
                <input name="id" type="hidden" value={porteriaId} />
                <button
                ...
                >
                    ...
                    >
                        ...
                        <p className="mt-3 text-base leading-7 text-slate-600">
                            Esta acción no se puede deshacer y puede bloquearse si la portería ya tiene eventos de acceso relacionados.
                        </p>
                        <div className="mt-5 rounded-[24px] border border-slate-200/80 bg-slate-50/80 px-4 py-4">
                            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">
                                Portería seleccionada
                            </p>
